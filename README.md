...
- 中国区 Bluedot/高德 CDN 承载的 `/clls/wloc` 响应也会被纳入 MITM 覆盖。
...
模块拦截 Apple Wi-Fi/蜂窝定位接口，以及中国区 Bluedot/高德 CDN 承载的同一路径：
...
- `ios-location-spoofer-dev.sgmodule`：开发诊断模块，会继续改写定位响应，并输出完整 raw 日志。
...
`dev` 会继续把响应改成 Apple Park，同时输出请求原文、Shadowrocket 暴露的原始响应、
解析用原响应、改写后响应的 base64 分块日志。它还会输出请求/响应 headers。
这个模块只适合临时调试；启用它时不要同时启用主模块。
