...
- `ios-location-spoofer-raw-dump.sgmodule`：完整 raw body base64 诊断模块。
...
`raw-dump` 模块不会修改流量，但会把 `/clls/wloc` 请求和响应 body 以 base64
分块完整写入日志。它会包含附近 Wi-Fi/BSSID 和蜂窝基站信息，只在需要深入排查时临时启用。
